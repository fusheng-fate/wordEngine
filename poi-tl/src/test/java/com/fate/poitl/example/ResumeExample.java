...
public class ResumeExample {
    ...
    public void init() throws FileNotFoundException {
        datas.setPortrait(Pictures.ofLocal(ResourceUtils.getFile("classpath:wordTemplate/image/sayis.png").getAbsolutePath()).size(100, 100).create());
        datas.setName("卅一");
        datas.setJob("BUG工程师");
        ...
        experiences.add(data1);
        datas.setExperience(
                Includes.ofLocal(ResourceUtils.getFile("classpath:wordTemplate/text/segments.docx").getAbsolutePath()).setRenderModel(experiences).create());
    }

    @Test
    public void testResumeExample() throws Exception {
        XWPFTemplate template = XWPFTemplate.compile(ResourceUtils.getFile("classpath:wordTemplate/text/resumes.docx")).render(datas);
